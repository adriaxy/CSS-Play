...
export default function CardLevel({levelName, levelId, levelPath, onClick, children, completedCount = 0, totalCount = 0}){
    ...
    return(
        <article className='card-level'>
            ...
            <div>{children}</div>
            <Link href={levelPath}>
                <button className='card-level__play' aria-label={`Play ${levelName}`}>
                    <svg className="play__svg" width="18" height="20" viewBox="0 0 16 18" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
